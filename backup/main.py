...
class Craft:
    ...
    def toStr(value):
        ...
        return ["Forestcraft", "Swordcraft", "Runecraft", "Dragoncraft",
                "Shadowcraft", "Bloodcraft", "Havencraft"][value]
...
def calcStats():
    ...
    for run in arenaRuns:
        ...
        print("type: {}, val: {}".format(run.gold.__class__, run.gold))
    for i in range(8):
        if stats["runs"][i] == 0:
            continue
        ...
        print("i: {}, gold: {}, runs: {}, avg: {}".format(i, totalGold[i], stats["runs"][i], stats["avggold"][i]))
    return stats
...
def printStats():
    ...
    print("Total\t{:.2f}\t\t{}\t\t{:.2f}".format(stats["avggold"][Craft.all], stats["winrate"][Craft.all], stats["avgwins"][Craft.all]))
